...
func (c cache) GetFrame(ctx context.Context, objectPath string, offsetU int64, frameTable *FrameTable, decompress bool, buf []byte, onProgress func(totalWritten int64)) (Range, error) {
	// For now, delegate directly to inner. NFS frame caching is added in storage_cache_frame.go.
	return c.inner.GetFrame(ctx, objectPath, offsetU, frameTable, decompress, buf, onProgress)
}
