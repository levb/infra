...
func (r *decompressReader) Close(ctx context.Context) (*ReadStats, error) {
	...
	stats := &ReadStats{
		CompressedBytes:   r.meteredIn.bytes.Load(),
		UncompressedBytes: r.meteredOut.bytes.Load(),
		Read:              time.Duration(r.meteredIn.nanos.Load()),
		Decompress:        max(0, time.Duration(r.meteredOut.nanos.Load())-time.Duration(r.meteredIn.nanos.Load())),
	}
