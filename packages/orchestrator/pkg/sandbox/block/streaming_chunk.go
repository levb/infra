...
func (c *Chunker) fetch(ctx context.Context, off, length int64, ft *storage.FrameTable) (storage.Source, error) {
	...
	if err != nil {
		return storage.SourceMmap, fmt.Errorf("failed to locate chunk for offset %d: %w", off, err)
	}
...
func (c *Chunker) progressiveRead(ctx context.Context, s *fetchSession, mmapSlice []byte, ft *storage.FrameTable) (stats *storage.ReadStats, source storage.Source, open time.Duration, err error) {
	...
	defer func() {
		...
		// Uncompressed reads emit a zero-duration decompress so the pipeline
		// shape matches the compressed path; compressed paths emit from the
		// decoder's Close.
		if !ft.IsCompressed() && stats != nil {
			storage.RecordReadDecompress(ctx, 0, stats.UncompressedBytes, okAttrs)
		}
	}()
