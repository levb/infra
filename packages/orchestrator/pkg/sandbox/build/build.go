...
var (
	fileReadAtTimer = utils.Must(telemetry.NewFloatTimerFactory(meter, "orchestrator.file.read_at",
		"Time to serve a build.File ReadAt across all source builds",
		"Bytes read", "ReadAt call count"))
	fileSliceTimer = utils.Must(telemetry.NewFloatTimerFactory(meter, "orchestrator.file.slice",
		"Time to serve a build.File Slice across all source builds",
		"Bytes returned", "Slice call count"))
)
...
func (b *File) Slice(ctx context.Context, off, length int64) ([]byte, error) {
	start := time.Now()
	out, err := b.slice(ctx, off, length)
	if err != nil {
		fileSliceTimer.Record(ctx, time.Since(start), 0, b.errAttrs(err))

		return nil, err
	}
	fileSliceTimer.Record(ctx, time.Since(start), int64(len(out)), b.okAttrs)
